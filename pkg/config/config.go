...
// UseLocalConfiguration set to true will add defaults that enable a Surogate Hub run
...
type OIDC struct {
	// configure how users are handled on the Surogate Hub side:
...
type CookieAuthVerification struct {
	...
	ValidateIDTokenClaims map[string]string `mapstructure:"validate_id_token_claims"`
	// DefaultInitialGroups is a list of groups to add to the user on the Surogate Hub side
	DefaultInitialGroups []string `mapstructure:"default_initial_groups"`
	// InitialGroupsClaimName comma separated list of groups to add to the user on the Surogate Hub side
	InitialGroupsClaimName string `mapstructure:"initial_groups_claim_name"`
...
func (b *Blockstore) BlockstoreLocalParams() (blockparams.Local, error) {
	...
	if err != nil {
		return blockparams.Local{}, fmt.Errorf("get absolute path for blockstore location %s: %w", path, err)
	}
