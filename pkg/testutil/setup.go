...
func SetupTestingEnv(params *SetupTestingEnvParams) (logging.Logger, apigen.ClientWithResponsesInterface, *s3.Client, string) {
	...
	if setupSgHub {
		...
		if err != nil {
			logger.WithError(err).Fatal("Failed to setup Surogate Hub")
		}
		if commResp.StatusCode() != http.StatusOK {
			logger.WithField("status", commResp.HTTPResponse.Status).Fatal("Failed to setup Surogate Hub")
		}
		...
		if res.StatusCode() != http.StatusOK {
			logger.WithField("status", res.HTTPResponse.Status).Fatal("Failed to setup Surogate Hub")
		}
