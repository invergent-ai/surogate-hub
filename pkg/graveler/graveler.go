...
// FirstCommitMsg is the message of the first (zero) commit of a Surogate Hub repository
...
func (g *Graveler) LoadCommits(ctx context.Context, repository *RepositoryRecord, metaRangeID MetaRangeID, opts ...SetOptionsFunc) error {
	...
	for iter.Next() {
		...
		if commit.GetGeneration() == 0 {
			return fmt.Errorf("dumps created by Surogate Hub versions before v0.61.0 are no longer supported: %w", ErrNoCommitGeneration)
		}
		commitID, err := g.RefManager.AddCommit(ctx, repository, Commit{
