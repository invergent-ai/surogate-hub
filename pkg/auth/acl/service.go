...
import (
	...
	"github.com/invergent-ai/surogate-hub/pkg/auth/keys"
	"github.com/invergent-ai/surogate-hub/pkg/auth/model"
	"github.com/invergent-ai/surogate-hub/pkg/auth/params"
...
// evictGroupMembers evicts cached user-policies entries for every member of the
// given group. Must be called while the group still exists (i.e. before deletion).
func (s *AuthService) evictGroupMembers(ctx context.Context, groupID string) {
	if !s.cache.Enabled() {
		return
	}
	...
	for it.Next() {
		...
		s.cache.EvictUserPolicies(user.Username)
	}
}
...
func (s *AuthService) DeleteUser(ctx context.Context, username string) error {
	...
	s.cache.EvictUser(auth.UserKey{Username: username})
	if user != nil {
		if user.Email != nil && *user.Email != "" {
			s.cache.EvictUser(auth.UserKey{Email: *user.Email})
		}
		if user.ExternalID != nil && *user.ExternalID != "" {
			s.cache.EvictUser(auth.UserKey{ExternalID: *user.ExternalID})
		}
	}
	s.cache.EvictUserPolicies(username)
	return err
}
...
// DetachPolicyFromUserNoValidation is an internal helper used in bulk-delete
// loops (DeleteUser, DeletePolicy). It does not evict the user-policies cache;
// callers that care are expected to drive their own eviction strategy.
...
func (s *AuthService) DetachPolicyFromUser(ctx context.Context, policyDisplayName, username string) error {
	...
	if err := s.DetachPolicyFromUserNoValidation(ctx, policyDisplayName, username); err != nil {
		return err
	}
	s.cache.EvictUserPolicies(username)
	return nil
...
func (s *AuthService) DeleteGroup(ctx context.Context, groupID string) error {
	...
	// Evict member policies while the group still exists.
	s.evictGroupMembers(ctx, groupID)
...
// removeUserFromGroupNoValidation is an internal helper used in bulk-delete
// loops (DeleteUser, DeleteGroup). It does not evict the user-policies cache;
// callers drive their own eviction strategy.
...
func (s *AuthService) RemoveUserFromGroup(ctx context.Context, username, groupID string) error {
	...
	if err := s.removeUserFromGroupNoValidation(ctx, username, groupID); err != nil {
		return err
	}
	s.cache.EvictUserPolicies(username)
	return nil
...
func (s *AuthService) DetachPolicyFromGroupIfAttached(ctx context.Context, policyDisplayName, groupDisplayName string) (bool, error) {
	pg := model.GroupPolicyPath(groupDisplayName, policyDisplayName)
	// Probe the secondary-index entry so we only incur member eviction for
	// groups that actually held this policy.
	var probe kv.SecondaryIndex
	_, err := kv.GetMsg(ctx, s.store, model.PartitionKey, pg, &probe)
	if errors.Is(err, kv.ErrNotFound) {
		return false, nil
	}
	if err != nil {
	...
	if err := s.DetachPolicyFromGroupNoValidation(ctx, policyDisplayName, groupDisplayName); err != nil {
		return false, err
	}
	return true, nil
}

func (s *AuthService) DetachPolicyFromUserIfAttached(ctx context.Context, policyDisplayName, username string) (bool, error) {
	pu := model.UserPolicyPath(username, policyDisplayName)
	var probe kv.SecondaryIndex
	_, err := kv.GetMsg(ctx, s.store, model.PartitionKey, pu, &probe)
	if errors.Is(err, kv.ErrNotFound) {
		return false, nil
	}
	if err != nil {
...
func (s *AuthService) DeletePolicy(ctx context.Context, policyDisplayName string) error {
	...
	// Detach from users that actually hold the policy; evict only those.
	...
	for it.Next() {
		entry := it.Entry()
		user := entry.Value.(*model.UserData)
		detached, err := s.DetachPolicyFromUserIfAttached(ctx, policyDisplayName, user.Username)
		if err != nil {
			return err
		}
		if detached {
	...
	// Detach from groups that actually hold the policy; evict only those groups' members.
	groupKey := model.GroupPath("")
	...
	for it.Next() {
		entry := it.Entry()
		group := entry.Value.(*model.GroupData)
		detached, err := s.DetachPolicyFromGroupIfAttached(ctx, policyDisplayName, group.DisplayName)
		if err != nil {
			return err
		}
		if detached {
			s.evictGroupMembers(ctx, group.DisplayName)
		}
	}
...
func (s *AuthService) DeleteCredentials(ctx context.Context, username, accessKeyID string) error {
	...
	s.cache.EvictCredential(accessKeyID)
	return nil
}
...
func (s *AuthService) AttachPolicyToGroup(ctx context.Context, policyDisplayName, groupDisplayName string) error {
	...
	s.evictGroupMembers(ctx, groupDisplayName)
	return nil
}

// DetachPolicyFromGroupNoValidation is an internal helper used in bulk-delete
// loops (DeletePolicy). It does not evict group members' caches; callers drive
// their own eviction strategy.
