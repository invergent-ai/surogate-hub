...
local function assert_lakefs_stats(table_names, content_paths)
    for _, table_path in ipairs(table_names) do
        ...
        if not table then
            error("missing Surogate Hub stat_object call for table path: " .. table_path .. "\n")
        end
        for _, data_path in ipairs(content_paths) do
            if not table[data_path] then
                error("missing Surogate Hub stat_object call for data path: " .. data_path .. " in table path: " .. table_path .. "\n")
            end
        end
    end
end
