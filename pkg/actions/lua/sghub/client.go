...
var LuaClientUserAgent = "sghub-lua/" + version.Version
...
func newHubRequest(ctx context.Context, user *model.User, method, reqURL string, data []byte) (*http.Request, error) {
...
func newHubJSONRequest(ctx context.Context, user *model.User, method, reqURL string, data []byte) (*http.Request, error) {
	req, err := newHubRequest(ctx, user, method, reqURL, data)
	if err != nil {
		return nil, err
	}
...
func getHubJSONResponse(l *lua.State, server *http.Server, request *http.Request) int {
...
func OpenClient(l *lua.State, ctx context.Context, user *model.User, server *http.Server) {
	clientOpen := func(l *lua.State) int {
		lua.NewLibrary(l, []lua.RegistryFunction{
			{Name: "create_tag", Function: func(state *lua.State) int {
				...
				req, err := newHubJSONRequest(ctx, user, http.MethodPost, reqURL, data)
				if err != nil {
					check(l, err)
				}
				return getHubJSONResponse(l, server, req)
			}},
			{Name: "diff_refs", Function: func(state *lua.State) int {
				...
				req, err := newHubJSONRequest(ctx, user, http.MethodGet, reqURL, nil)
				if err != nil {
					check(l, err)
				}
				// query params
	...
	lua.Require(l, "sghub", clientOpen, false)
	l.Pop(1)
}
