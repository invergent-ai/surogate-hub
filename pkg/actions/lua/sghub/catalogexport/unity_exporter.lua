--[[ TABLE SPECIFICATION:   _hub_tables/<table path>
name: <table name>
type: delta
...
--[[
    - table_descriptors_path: the path under which the table descriptors reside (e.g. "_hub_tables").
...
local function register_tables(action, table_descriptors_path, delta_table_details, databricks_client, warehouse_id)
    ...
    for table_name_yaml, table_details in pairs(delta_table_details) do
        ...
        local table_descriptor = extractor.get_table_descriptor(sghub, repo, commit_id, table_src_path)
