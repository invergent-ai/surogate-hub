local extractor = require("sghub/catalogexport/table_extractor")
local hive = require("sghub/catalogexport/hive")
local utils = require("sghub/catalogexport/internal")
local pathlib = require("path")
local strings = require("strings")
local sghub = require("sghub")

--[[
    @repo_id: repository id
    @commit_id: commit id of the current table
    @table_src_path: path to table spec (i.e _hub_tables/my_table.yaml)
    @options:
...
local function export_it(repo_id, commit_id, table_src_path, options)
    ...
    local descriptor = extractor.get_table_descriptor(sghub, repo_id, commit_id, table_src_path)
    if descriptor.type ~= "hive" then
        error("table " .. descriptor.type .. " in path " .. table_src_path .. " not supported")
    end
    if opts.debug then
        print(string.format('%s table `sg://%s/%s/%s`', descriptor.type, repo_id, utils.short_digest(commit_id),
            descriptor.path))
    end
    local base_path = descriptor.path
    local cols = descriptor.partition_columns
    local pager = hive.extract_partition_pager(sghub, repo_id, commit_id, base_path, cols)
    return function()
...
--[[
    ...
    @table_src_path: object path to the table spec (_hub_tables/my_table.yaml)
