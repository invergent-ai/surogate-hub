...
func (c *Controller) LinkPhysicalAddress(w http.ResponseWriter, r *http.Request, body apigen.LinkPhysicalAddressJSONRequestBody, owner, repository, branch string, params apigen.LinkPhysicalAddressParams) {
	...
	if addressType == catalog.AddressTypeRelative {
		// if the address is in the storage namespace, verify it has been produced by Surogate Hub
		if err = c.Catalog.VerifyLinkAddress(repository, branch, params.Path, physicalAddress); c.handleAPIError(ctx, w, r, err) {
			return
		}
...
func (c *Controller) ListRepositories(w http.ResponseWriter, r *http.Request, params apigen.ListRepositoriesParams) {
	...
	for _, repo := range repos {
		...
		if !c.canReadRepository(ctx, repo.Name) {
			continue
		}
		...
		r := apigen.Repository{
			...
			Metadata:         &apigen.RepositoryMetadata{AdditionalProperties: repo.Metadata},
		}
...
func (c *Controller) CreateRepository(w http.ResponseWriter, r *http.Request, body apigen.CreateRepositoryJSONRequestBody, params apigen.CreateRepositoryParams) {
	...
	if body.Metadata != nil {
		err = c.Catalog.UpdateRepositoryMetadata(ctx, newRepo.Name, body.Metadata.AdditionalProperties)
		if c.handleAPIError(ctx, w, r, err) {
			return
		}
		response.Metadata = &apigen.RepositoryMetadata{AdditionalProperties: body.Metadata.AdditionalProperties}
	}
...
func (c *Controller) ensureStorageNamespace(ctx context.Context, storageID, storageNamespace string) error {
	const (
		dummyData    = "this is dummy data - created by Surogate Hub to check accessibility"
		dummyObjName = "dummy"
	)
	...
	// this serves two purposes, first, we maintain safety check for older Surogate Hub version.
	// second, in scenarios where Surogate Hub shouldn't have access to the root namespace (i.e pre-sign URL only).
	if c.Config.GetBaseConfig().Graveler.EnsureReadableRootNamespace {
		...
		if s, err := c.BlockAdapter.Get(ctx, rootObj); err == nil {
			_ = s.Close()
			return fmt.Errorf("found Surogate Hub objects in the storage root (%s:%s): %w",
...
func (c *Controller) DeleteRepository(w http.ResponseWriter, r *http.Request, owner string, repository string, params apigen.DeleteRepositoryParams) {
	...
	if repo != nil && repo.ReadOnly && !swag.BoolValue(params.Force) {
		if c.handleAPIError(ctx, w, r, graveler.ErrReadOnlyRepository) {
			return
		}
	}
...
func (c *Controller) GetRepository(w http.ResponseWriter, r *http.Request, owner string, repository string) {
	...
	switch {
	case err == nil:
		metadata, errMeta := c.Catalog.GetRepositoryMetadata(ctx, repository)
		if c.handleAPIError(ctx, w, r, errMeta) {
			return
		}
		...
		response := apigen.Repository{
			...
			StorageNamespace: repo.StorageNamespace,
			ReadOnly:         swag.Bool(repo.ReadOnly),
			Metadata:         &apigen.RepositoryMetadata{AdditionalProperties: metadata},
		}
...
func (c *Controller) GetSetupState(w http.ResponseWriter, r *http.Request) {
	...
	case errors.Is(err, auth.ErrNotFound):
		...
		// 1. The setup ran on an older version of Surogate Hub that didn't have commprefs. In this case, we treat it as set.
		// 2. The setup ran on a newer version of Surogate Hub that has commprefs, but the setup didn't complete. In this case, we treat it as not set.
...
func (c *Controller) Setup(w http.ResponseWriter, r *http.Request, body apigen.SetupJSONRequestBody) {
	...
	if initialized {
		writeError(w, r, http.StatusConflict, "Surogate Hub already initialized")
		return
	}
...
func (c *Controller) isExternalPrincipalNotSupported(_ context.Context) bool {
	return true
}

func (c *Controller) canReadRepository(ctx context.Context, repository string) bool {
	user, err := auth.GetUser(ctx)
	if err != nil {
		return false
	}
