// Package dockertest provides a wrapper for running a Surogate Hub container in
// integration tests.
package dockertest
...
const (
	DockerSgHubRepository   = "invergent-ai/surogate-hub"
...
// Build holds parameters for building a Surogate Hub dockertest container.
type Build struct {
	Repository string
	...
	// use for the Surogate Hub local block adapter.  By default it is a
...
func (b *Build) New(ctx context.Context, t testing.TB, pool *dockertest.Pool) *Container {
	t.Helper()
	if b.Repository == "" {
		b.Repository = DockerSgHubRepository
	}
	if b.DockerTag == "" {
	...
	return newHubContainer(ctx, b, t, pool)
}

// Container holds a Surogate Hub dockertest container.
type Container struct {
	...
	// container and used by the Surogate Hub local block adapter.
	LocalDirectory string
...
func newHubContainer(ctx context.Context, b *Build, t testing.TB, pool *dockertest.Pool) *Container {
	var (
		ret Container
		err error
	)
	...
	defer func() {
		if err != nil && ret.Resource != nil {
			...
			if closeErr != nil {
				t.Logf("While closing Surogate Hub container after error: %s", closeErr)
			}
		}
	}()
	...
	copy(env, b.Env)
	env = append(env,
		"SGHUB_STATS_ENABLED=0",
		"SGHUB_AUTH_ENCRYPT_SECRET_KEY=shh...",
		"SGHUB_BLOCKSTORE_TYPE=mem",
		"SGHUB_DATABASE_TYPE=local",
	)
	...
	if err != nil {
		t.Fatalf("Failed to start Surogate Hub container: %s", err)
	}

	err = ret.Resource.Expire(ContainerTimeoutSeconds)
	if err != nil {
		t.Errorf("Set Surogate Hub container expiry to %d: %s (keep going, it's already running)", ContainerTimeoutSeconds, err)
	}
