...
func StreamRepositoryDiffs(ctx context.Context, client apigen.ClientWithResponsesInterface, left, right *uri.URI, prefix string, diffs chan<- apigen.Diff, twoDot bool) error {
	...
	var after string
	for hasMore {
		diffResp, err := client.DiffRefsWithResponse(ctx, apigen.RepositoryOwner(left.Repository), apigen.RepositoryName(left.Repository), left.Ref, right.Ref, &apigen.DiffRefsParams{
