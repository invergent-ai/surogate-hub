...
from surogate_hub.exceptions import NotFoundException, ObjectExistsException
from surogate_hub.object import (
    ObjectWriter,
    ReadModes,
    StoredObject,
    WriteableObject,
...
def test_writer(setup_repo):
    ...
    with obj.writer() as writer:
        ...
        # Check that the object does not exist in Surogate Hub before we close the writer
...
def test_reader_with_failure(setup_repo):
    ...
    # Check that the object does not exist in Surogate Hub after exception
...
def test_large_file_object_write(setup_repo, w_mode):
    ...
    # set in SpooledTemporaryFile (in python-wrapper/surogate_hub/object.py)
    # to force disk write
    #
