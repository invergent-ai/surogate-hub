...
import surogate_hub
from surogate_hub.exceptions import BadRequestException
...
def test_reference_diff(setup_branch_with_commits):
    ...
    other_branch = surogate_hub.Repository(branch.repo_id).branch("other_branch").create("test_branch")
...
def test_reference_merge_into(setup_branch_with_commits):
    branch = setup_branch_with_commits
    repo = surogate_hub.Repository(branch.repo_id)
