...
from surogate_hub.exceptions import NotFoundException, ConflictException, ObjectNotFoundException
import surogate_hub


def test_repository_sanity(storage_namespace, setup_repo):
    _, repo = setup_repo
    repo = surogate_hub.repository(repo.properties.id)  # test the surogate_hub.repository function works properly
    default_branch = "main"
    expected_properties = surogate_hub.RepositoryProperties(id=repo.properties.id,
...
def test_object_sanity(setup_repo):
    ...
    path = "test_obj"
    metadata = {"foo": "bar"}
    obj = surogate_hub.WriteableObject(repository_id=repo.properties.id, reference_id="main", path=path, client=clt).upload(
...
def test_ssl_configuration(ssl_enabled):
    ...
    cert = "somecert"
    clt = surogate_hub.client.Client(verify_ssl=ssl_enabled, proxy=proxy, ssl_ca_cert=cert)
