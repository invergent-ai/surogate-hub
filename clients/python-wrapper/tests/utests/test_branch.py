...
from surogate_hub.exceptions import ConflictException
...
def test_branch_create(monkeypatch):
    ...
    with monkeypatch.context():
        def monkey_create_branch(_self, repo_name, branch_creation, *_):
            ...
            return surogate_hub_sdk.BranchCreation(name=branch.id, source=source)

        monkeypatch.setattr(surogate_hub_sdk.BranchesApi, "create_branch", monkey_create_branch)
...
def test_branch_head(monkeypatch):
    ...
    with monkeypatch.context():
        def monkey_get_branch(repo_name, branch_name, *_):
            ...
            return surogate_hub_sdk.Ref(commit_id=commit_id, id=branch.id)
...
def test_branch_commit(monkeypatch):
    ...
    with monkeypatch.context():
        def monkey_commit(repo_name, branch_name, commits_creation, *_):
            ...
            return surogate_hub_sdk.Commit(
                id=commit_id,
                parents=[""],
                committer="Committer",
...
def test_branch_revert(monkeypatch):
    ...
    with monkeypatch.context():
        ...
        with pytest.warns(surogate_hub.LakeFSDeprecationWarning, match="reference_id is deprecated.*"):
        ...
        with pytest.raises(ValueError, match="`reference_id` and `reference` both provided.*"), \
                pytest.warns(surogate_hub.LakeFSDeprecationWarning, match="reference_id is deprecated.*"):
