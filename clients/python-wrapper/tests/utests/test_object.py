...
import surogate_hub_sdk.api

from surogate_hub.object import ReadModes
...
class StorageTestConfig(surogate_hub_sdk.StorageConfig):
...
class ObjectTestStats(surogate_hub_sdk.ObjectStats):
    def __init__(self) -> None:
        super().__init__(path="",
                         path_type="object",
                         physical_address="",
...
class StagingTestLocation(surogate_hub_sdk.StagingLocation):
    def __init__(self) -> None:
...
def readable_object_context(monkey, **kwargs):
    with monkey.context():
        from surogate_hub.object import StoredObject
        clt = get_test_client()
        conf = surogate_hub_sdk.Config(version_config=surogate_hub_sdk.VersionConfig(), storage_config=StorageTestConfig())
...
def writeable_object_context(monkey, **kwargs):
    with monkey.context():
        monkey.setattr(surogate_hub_sdk.api.BranchesApi, "get_branch", lambda *args: None)
        from surogate_hub.object import WriteableObject
        conf = surogate_hub_sdk.Config(version_config=surogate_hub_sdk.VersionConfig(), storage_config=StorageTestConfig())
        clt = get_test_client()
        monkey.setattr(clt, "_server_conf", conf)
...
class TestStoredObject:
    def test_exists(self, monkeypatch, tmp_path):
        ...
        with readable_object_context(monkeypatch, **test_kwargs.__dict__) as obj:
            # Object exists
            monkeypatch.setattr(surogate_hub_sdk.api.ObjectsApi, "head_object", lambda *args: None)
            assert obj.exists()
            # Object doesn't exist
            monkeypatch.setattr(surogate_hub_sdk.api.ObjectsApi, "head_object",
                                lambda *args: (_ for _ in ()).throw(surogate_hub_sdk.exceptions.NotFoundException(
...
class TestObjectReader:
    ...
    def verify_reader(fd, patch_setattr, test_kwargs, data):
        ...
        patch_setattr(surogate_hub_sdk.api.ObjectsApi, "stat_object", lambda *args: object_stats)
        ...
        patch_setattr(surogate_hub_sdk.api.ObjectsApi, "get_object", monkey_get_object)
        assert fd.read() == data
    ...
    def test_read_modes(self, monkeypatch, tmp_path, mode):
        ...
        with readable_object_context(monkeypatch, **test_kwargs.__dict__) as obj:
            with obj.reader(mode=mode) as fd:
                ...
                object_stats.path = test_kwargs.path
                object_stats.size_bytes = len(data)
                monkeypatch.setattr(surogate_hub_sdk.api.ObjectsApi, "stat_object", lambda *args: object_stats)
                ...
                monkeypatch.setattr(surogate_hub_sdk.api.ObjectsApi, "get_object", monkey_get_object)
                res = fd.read()
                if 'b' not in mode:
...
class TestWriteableObject:
    def test_upload(self, monkeypatch, tmp_path):
        ...
        with writeable_object_context(monkeypatch, **test_kwargs.__dict__) as obj:
            ...
            monkeypatch.setattr(surogate_hub_sdk.api.StagingApi, "get_physical_address", lambda *args: staging_location)
            monkeypatch.setattr(urllib3.PoolManager, "request",
                                lambda *args, **kwargs: urllib3.response.HTTPResponse(status=201))

            def monkey_link_physical_address(*_, staging_metadata: surogate_hub_sdk.StagingMetadata, **__):
                assert staging_metadata.size_bytes == len(data)
                assert staging_metadata.staging == staging_location
                return surogate_hub_sdk.ObjectStats(path=obj.path,
                                              path_type="object",
                                              physical_address=staging_location.physical_address,
                                              checksum="",
                                              mtime=12345)

            monkeypatch.setattr(surogate_hub_sdk.api.StagingApi, "link_physical_address", monkey_link_physical_address)
            # Test string
            data = "test_data"
