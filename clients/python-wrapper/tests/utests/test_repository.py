...
from surogate_hub.exceptions import (
    ServerException,
...
from surogate_hub import RepositoryProperties
...
def monkey_create_repository(_self, repository_creation, *_):
    ...
    return surogate_hub_sdk.Repository(id=TEST_REPO_ARGS.name,
...
def test_repository_creation(monkeypatch):
    ...
    with monkeypatch.context():
        monkeypatch.setattr(surogate_hub_sdk.RepositoriesApi, "create_repository", monkey_create_repository)
...
def test_repository_creation_already_exists(monkeypatch):
    repo = get_test_repo()
    ex = surogate_hub_sdk.exceptions.ApiException(status=http.HTTPStatus.CONFLICT.value)

    with monkeypatch.context():
        def monkey_create_repository_ex(*_):
            raise ex

        monkeypatch.setattr(surogate_hub_sdk.RepositoriesApi, "create_repository", monkey_create_repository_ex)

        # Expect success when exist_ok = True
        existing = surogate_hub_sdk.Repository(id=TEST_REPO_ARGS.name,
                                         default_branch="main",
        ...
        monkeypatch.setattr(surogate_hub_sdk.RepositoriesApi, "get_repository", monkey_get_repository)
    ...
    # Expect fail on exists
    ex = surogate_hub_sdk.exceptions.UnauthorizedException(http.HTTPStatus.UNAUTHORIZED)
...
def test_delete_repository(monkeypatch):
    repo = get_test_repo()
    with monkeypatch.context():
        monkeypatch.setattr(surogate_hub_sdk.RepositoriesApi, "delete_repository", lambda *args: None)
        repo.delete()
        ...
        monkeypatch.setattr(surogate_hub_sdk.RepositoriesApi, "delete_repository", monkey_delete_repository)
        # Not found
        ex = surogate_hub_sdk.exceptions.NotFoundException(status=http.HTTPStatus.NOT_FOUND)
        with expect_exception_context(NotFoundException):
            repo.delete()

        # Unauthorized
        ex = surogate_hub_sdk.exceptions.UnauthorizedException(status=http.HTTPStatus.UNAUTHORIZED)
        with expect_exception_context(NotAuthorizedException):
            repo.delete()

        # Other error
        ex = surogate_hub_sdk.exceptions.ApiException()
...
def test_create_repository_no_authentication(monkeypatch):
    with lakectl_no_config_context(monkeypatch):
        from surogate_hub.repository import Repository
        ...
        with env_var_context():
            from surogate_hub import config as client_config
