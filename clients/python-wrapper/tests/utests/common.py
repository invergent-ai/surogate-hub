...
import surogate_hub.repository
...
TEST_REPO_ARGS = surogate_hub_sdk.RepositoryCreation(name="test-repo",
...
def lakectl_test_config_context(monkey, tmp_path):
    ...
    with monkey.context():
        ...
        from surogate_hub import client  # Must be imported after the monkey patching
...
def get_test_repo() -> surogate_hub.Repository:
    from surogate_hub.client import Client
    client = Client(username="test_user", password="test_password", host="http://127.0.0.1:8000")
    return surogate_hub.Repository(repository_id=TEST_REPO_ARGS.name, client=client)


@contextmanager
def expect_exception_context(ex, message=None):
    try:
        yield
