...
from surogate_hub import ObjectInfo, CommonPrefix
...
def test_reference_log(monkeypatch):
    ...
    def monkey_log_commits(*_, **__):
        ...
        for pid in range(items_per_page):
            ...
            results.append(surogate_hub_sdk.Commit(
                id=str(index),
                parents=[""],
        ...
        idx += 1
        pagination = surogate_hub_sdk.Pagination(
            has_more=idx < pages,
            next_offset="",
        ...
        return surogate_hub_sdk.CommitList(
...
def test_reference_diff(monkeypatch):
    ...
    def monkey_diff_refs(*_, **__):
        ...
        for pid in range(items_per_page):
            index = items_per_page * idx + pid
            results.append(surogate_hub_sdk.Diff(
                type="added",
                path=str(index),
                path_type="object",
                size_bytes=index,
        ...
        return surogate_hub_sdk.DiffList(
            pagination=pagination,
            results=results
        )
...
def test_reference_objects(monkeypatch):
    ...
    with monkeypatch.context():
        def monkey_list_objects(*_, **__):
            ...
            for i in range(10):
                if i % 2:
                    results.append(surogate_hub_sdk.ObjectStats(
                        path=f"path-{i}",
                        path_type="object",
            ...
            return surogate_hub_sdk.ObjectStatsList(pagination=surogate_hub_sdk.Pagination(
                has_more=False,
                next_offset="",
                max_per_page=1,
                results=1),
