...
- name: SGHUB_DATABASE_POSTGRES_CONNECTION_STRING
  valueFrom:
    secretKeyRef:
...
- name: SGHUB_AUTH_ENCRYPT_SECRET_KEY
  valueFrom:
    secretKeyRef:
      name: {{ .Values.existingSecret }}
...
- name: SGHUB_GATEWAYS_S3_FALLBACK_URL
  value: http://localhost:7001
{{- end }}
{{- if .Values.committedLocalCacheVolume }}
- name: SGHUB_COMMITTED_LOCAL_CACHE_DIR
  value: /shub/cache
{{- end }}
{{- if .Values.useDevPostgres }}
- name: SGHUB_DATABASE_TYPE
  value: postgres
- name: SGHUB_DATABASE_POSTGRES_CONNECTION_STRING
