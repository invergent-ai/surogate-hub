...
func TestHubctlUserAgentString(t *testing.T) {
	osInfo := osinfo.GetOSInfo()
	expectedUserAgent := fmt.Sprintf("hubctl/%s/%s/%s/%s", version.Version, osInfo.OS, osInfo.Version, osInfo.Platform)
	...
	t.Setenv("HUBCTL_SERVER_ENDPOINT_URL", server.URL)
	rootCmd.SetArgs([]string{"--version"})
	require.NoError(t, rootCmd.Execute())
}

func TestHubctlUnixPerm(t *testing.T) {
	trueP := "true"
	falseP := "false"
	testCases := []struct {
		Name   string
	...
	for _, tt := range testCases {
		t.Run(tt.Name, func(t *testing.T) {
			...
			if len(expectedVal) > 0 {
				t.Setenv("HUBCTL_EXPERIMENTAL_LOCAL_POSIX_PERMISSIONS_ENABLED", expectedVal)
			}
