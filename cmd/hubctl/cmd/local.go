...
const (
	...
	CaseInsensitiveWarningMessageFormat = `Directory '%s' is case-insensitive, versioning tools such as hubctl local and git will work incorrectly.`
)
...
func localHandleSyncInterrupt(ctx context.Context, idx *local.Index, operation string) context.Context {
	...
	Use "hubctl local checkout..." to sync with the remote.`, 1)
	}()
	return ctx
}
...
Use "hubctl local commit..." to commit your latest changes or "hubctl local pull... --force" to sync with the remote.`, 1)
...
Use "hubctl local pull... --force" to sync with the remote.`, 1)
		case cloneOperation:
			Die(`Latest clone operation was interrupted, local data may be incomplete.
Use "hubctl local checkout..." to sync with the remote or run "hubctl local clone..." with a different directory to sync with the remote.`, 1)
		default:
...
var localCmd = &cobra.Command{
	Use:   "local",
	Short: "Sync local directories with Surogate Hub paths",
