...
var localCheckoutCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		fmt.Println("the operation will revert all changes in all directories that are linked with Surogate Hub.")
...
func localCheckout(cmd *cobra.Command, localPath string, specifiedRef string, confirmByFlag bool) {
	...
	if err != nil {
		if errors.Is(err, fs.ErrNotExist) {
			DieFmt("directory %s is not linked to a Surogate Hub path", localPath)
		}
		DieErr(err)
	}
