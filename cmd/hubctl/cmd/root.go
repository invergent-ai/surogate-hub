...
type Configuration struct {
	...
	Local struct {
		// SkipNonRegularFiles - By default hubctl local fails if local directory contains a symbolic link. When set, hubctl will ignore the symbolic links instead.
		SkipNonRegularFiles bool `mapstructure:"skip_non_regular_files"`
	} `mapstructure:"local"`
	// Experimental - Use caution when enabling experimental features. It should only be used after consulting with the Surogate Hub team!
	Experimental struct {
		Local struct {
			POSIXPerm struct {
				Enabled    bool `mapstructure:"enabled"`
...
var rootCmd = &cobra.Command{
	Use:   "hubctl",
	Short: "A cli tool to explore manage and work with Surogate Hub",
	Long:  `hubctl is a CLI tool allowing exploration and manipulation of a Surogate Hub environment`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		info := versionInfo{HubctlVersion: version.Version}

		// get Surogate Hub server version
		...
		if err != nil {
			...
			if err != nil {
				...
				info.HubctlLatestVersion = latest.LatestVersion
				if info.UpgradeURL == "" {
...
func getHTTPClient() *http.Client {
	...
	// Ignore self-signed / invalid TLS certificates (user request). NOTE: This is insecure and should
	// only be used in development or trusted environments.
...
func getClient() *apigen.ClientWithResponses {
	...
	client, err := apigen.NewClientWithResponses(
		...
		apigen.WithRequestEditorFn(func(ctx context.Context, req *http.Request) error {
			...
			req.Header.Set("User-Agent", fmt.Sprintf("hubctl/%s/%s/%s/%s", version.Version, oss.OS, oss.Version, oss.Platform))
			return nil
		}),
	)
	if err != nil {
...
func init() {
	...
	rootCmd.PersistentFlags().StringVarP(&cfgFile, "config", "c", "", "config file (default is $HOME/.hubctl.yaml)")
	...
	rootCmd.PersistentFlags().StringVarP(&baseURI, "base-uri", "", os.Getenv("HUBCTL_BASE_URI"), "base URI used for Surogate Hub address parse")
	...
	rootCmd.Flags().BoolP("version", "v", false, "version for hubctl")
}
