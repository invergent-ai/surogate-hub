...
func localInit(ctx context.Context, dir string, remote *uri.URI, force, updateIgnore bool) (string, error) {
	...
	if remotePath != "" && !strings.HasSuffix(remotePath, uri.PathSeparator) { // Verify path is not an existing object
		...
		case stat.JSON200 != nil:
			DieFmt("Surogate Hub path %s is an existing object and cannot be used as a reference source", remote.String())
...
var localInitCmd = &cobra.Command{
	...
	Short: "set a local directory to sync with a Surogate Hub path.",
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err != nil {
			if errors.Is(err, fs.ErrExist) {
				DieFmt("directory '%s' already linked to a Surogate Hub path, run command with --force to overwrite", localPath)
			}
			DieErr(err)
		}
...
func init() {
	withForceFlag(localInitCmd, "Overwrites if directory already linked to a Surogate Hub path")
