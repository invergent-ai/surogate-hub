...
import (
	...
	authremote "github.com/invergent-ai/surogate-hub/pkg/auth/remoteauthenticator"
	...
	_ "github.com/invergent-ai/surogate-hub/pkg/kv/cosmosdb"
	_ "github.com/invergent-ai/surogate-hub/pkg/kv/dynamodb"
...
func NewAuthService(_ context.Context, cfg *config.BaseConfig, _ logging.Logger, kvStore kv.Store, _ *auth.KVMetadataManager) auth.Service {
...
var runCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if (kvParams.Type == local.DriverName || kvParams.Type == mem.DriverName) &&
			...
			setupCreds, err := setupSgHub(ctx, baseCfg, authMetadataManager, authService, baseCfg.Installation.UserName,
...
func checkRepos(ctx context.Context, logger logging.Logger, config config.Config, authMetadataManager auth.MetadataManager, blockStore block.Adapter, c *catalog.Catalog) {
	...
	if err != nil {
		logger.WithError(err).Fatal("Failed to check if Surogate Hub is initialized")
	}
	if !initialized {
		logger.Debug("Surogate Hub isn't initialized, skipping mismatched adapter checks")
	} else {
		logger.
			WithField("adapter_type", blockStore.BlockstoreType()).
			Debug("Surogate Hub is initialized, checking repositories for mismatched adapter")
		hasMore := true
		next := ""
...
// A foreign repo might exist if the Surogate Hub instance configuration changed after a repository was
// already created.
// The behavior of Surogate Hub for foreign repos is undefined and should be blocked.
func checkForeignRepo(repoStorageType block.StorageType, logger logging.Logger, adapterStorageType, repoName string) {
	if adapterStorageType != repoStorageType.BlockstoreType() {
		logger.Fatalf("Mismatched adapter detected. Surogate Hub started with adapter of type '%s', but repository '%s' is of type '%s'",
...
const runBannerTmpl = `
Surogate Hub {{ .Version }} - Up and running (^C to shutdown)...
{{ .SetupMessage }}
`

const localBanner = `
│ If you're running Surogate Hub locally for the first time,
│     complete the setup process at http://127.0.0.1:8000/setup`

var quickStartBanner = fmt.Sprintf(`
│ Surogate Hub running in quickstart mode. 
