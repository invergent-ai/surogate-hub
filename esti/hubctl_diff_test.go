...
func TestHubctlDiffAddedFiles(t *testing.T) {
...
func TestHubctlDiffDeletedFiles(t *testing.T) {
	repoName := generateUniqueRepositoryName()
	storage := generateUniqueStorageNamespace(repoName)
...
func TestHubctlDiffPrefix(t *testing.T) {
	repoName := generateUniqueRepositoryName()
	storage := generateUniqueStorageNamespace(repoName)
...
func runDiffAndExpect(t *testing.T, repoName string, testBranch string, prefix string, diffArgs *ExpectedDiff) {
	...
	RunCmdAndVerifySuccessWithFile(t, cmd.Get(), false, "hubctl_diff", diffVars)
}
