...
// hubctl tests utility functions
...
func hubctlLocation() string {
	return viper.GetString("binaries_dir") + "/hubctl"
}

func HubctlWithParams(accessKeyID, secretAccessKey, endPointURL string) string {
	return HubctlWithParamsWithPosixPerms(accessKeyID, secretAccessKey, endPointURL, false)
}

func HubctlWithParamsWithPosixPerms(accessKeyID, secretAccessKey, endPointURL string, withPosixPerms bool) string {
	hubctlCmdline := "HUBCTL_CREDENTIALS_ACCESS_KEY_ID=" + accessKeyID +
		" HUBCTL_CREDENTIALS_SECRET_ACCESS_KEY=" + secretAccessKey +
		" HUBCTL_SERVER_ENDPOINT_URL=" + endPointURL +
		" HUBCTL_EXPERIMENTAL_LOCAL_POSIX_PERMISSIONS_ENABLED=" + strconv.FormatBool(withPosixPerms) +
		" " + hubctlLocation()

	return hubctlCmdline
}

func Hubctl() string {
	return HubctlWithParams(viper.GetString("access_key_id"), viper.GetString("secret_access_key"), viper.GetString("endpoint_url"))
}

func HubctlWithPosixPerms() string {
	return HubctlWithParamsWithPosixPerms(viper.GetString("access_key_id"), viper.GetString("secret_access_key"), viper.GetString("endpoint_url"), true)
}
