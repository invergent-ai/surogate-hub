...
const NoContentDiff = ({left, right, diffType}) => {
    ...
    return <div>
        ...
        <span><Alert variant="light"><InfoIcon/> {`Surogate Hub supports content diff for ${supportedFileExtensions.join(',')} file formats only`}</Alert></span>
    </div>;
}
...
const DiffSizeReport = ({leftSize, rightSize, diffType}) => {
    ...
    switch (diffType) {
        case 'changed':
            ...
            break;
        case 'conflict': // conflict will compare left and right.
                return <div>
